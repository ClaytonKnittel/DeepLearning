...
class nn_mc:
    ...
    def __init__(self, C=.4):
        ...
        self.state_counts = {}
        # cumulative sum of scores of paths leading from this branch
        self.aggregate_score = {}
        self.true_scores = []
        self.pred_scores = []

    # average value of state given what has already been explored
    def avg_val(self, states):
        pass
    ...
    def visit(self, game_state, final_score):
        self.N += 1
        if game_state in self.state_counts:
            self.state_counts[game_state] += 1
            self.aggregate_score[game_state] += final_score
        else:
            cpy = game_state.copy()
            self.state_counts[cpy] = 1
            self.aggregate_score[cpy] = final_score

    def reset_state_tracking(self):
        self.state_counts = {}
        self.aggregate_score = {}

    # called for each game state we want to learn from
    ...
    def best_move(self, game_state):
        ...
        # square root of total number of visits to this game state
        sqrtn = sqrt(self.state_counts.get(game_state, 0))
        ...
        for move in moves:
            ...
            cnt = self.state_counts.get(game_state, 0)
            state_counts.append(cnt)
            total_count += cnt
            # if no scores yet, keep it neutral (tie state = .5)
            aggregate_scores.append(self.aggregate_score.get(game_state, .5))

            game_state.undo(move)

        # calculate average value of all states based on what has already been
        # explored
        hs = self.avg_val(states)
        hs = [(self.aggregate_score.get(state, 0) / self.state_counts.get(state, 1) + 1) / 2
                for state in states]
